...
{    
    ...
    while(strcmp(stack_top(stack), "$"))
    {
        ...
        printf("\n:::%s\n", current_node->next[0].data[0].data);
        next_count_count++;
        next_count_stack1[next_count_count] = table_linelength(rules, rule) - 1;
        printf("\naaaaaa  %d\n",next_count_stack1[next_count_count]);
        next_count_stack2[next_count_count] = 0;
        current_node = &current_node->next[next_count_stack2[next_count_count]];
        printf("------------%s\n",current_node->prev->data[0].data);
        if(!strcmp(token, stack_top(stack)))
        {
            current_node = current_node->prev;
            next_count_stack2[next_count_count]++;/*
            while(next_count_stack1[next_count_count] == next_count_stack2[next_count_count] && next_count_count > 0)
            {
                printf("\n\n%d\n", next_count_count);
                current_node = current_node->prev;
                next_count_count--;
    ...
    tree_print(node);
    printf("\n%s popop", node.next[0].next[0].next[0].next[0].next[0].data[0].data);
